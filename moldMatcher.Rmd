...
# devo iniziare a settare la pipeline per tradurre ttto quello che sta succedendo -> INIZIA CON LA PIPELINE FATTTA BEN
...
putative_222 <- putative_222 %>% dplyr::select(unique_id,names, seq_score, c_box_seq, d_prime_box_seq, c_prime_box_seq, d_box_seq,c_d_prime_dist, d_prime_c_prime_dist, c_prime_d_dist, c_d_dist)
putative_222$guide_score <- guides_scores[putative_222$seq_score]
putative_222$c_box_score <- cbox_scores[putative_222$c_box_seq]
...
```
alignments in terminal helix of scrambled snornas
```{r}
...
match_search <- function(i, snoRNA_seq, rRNA_seq){
  if (i-3 >0 & i+5<=width(rRNA_seq)){
    ...
    # temp <- temp[temp$guide_score >0.68]ù
    return(temp)
  }
}

met_sites$Site[met_sites$Site == "Ψ966"] <- "Ψm966"
